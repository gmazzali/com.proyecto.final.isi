...
public class ValidateAssessmentTestUnit {
	...
	public void pruebaDeLaValidacionCompleta() {
		...
		rule.setRule("[(?x rdf:type http://www.assessments.com/#MultipleChoiceInstrument) -> (?x rb:violation error('error', 'Error de multiple choice', ?x))]");
		set.addRule(rule);

		rule = new Rule();
		rule.setId(2);
		rule.setDescription("Regla que falla por simple choice");
		rule.setRule("[(?x rdf:type http://www.assessments.com/#SingleChoiceInstrument) -> (?x rb:violation error('error', 'Error de simple choice', ?x))]");
		set.addRule(rule);

		rule = new Rule();
		rule.setId(3);
		rule.setDescription("Regla que falla por correspondencia");
		rule.setRule("[(?x rdf:type http://www.assessments.com/#CorrespondenceInstrument) -> (?x rb:violation error('error', 'Error de correspondencia', ?x))]");
		set.addRule(rule);

		rule = new Rule();
		rule.setId(4);
		rule.setDescription("Regla que no falla por portfolio");
		rule.setRule("[(?x rdf:type http://www.assessments.com/#PortfolioInstrument) -> (?x rb:violation error('error', 'Error de portfolio', ?x))]");
		set.addRule(rule);

		rule = new Rule();
		rule.setId(5);
		rule.setDescription("Regla compleja de conteo de reactivos en actividad");
		rule.setRule("[(?a rdf:type http://www.assessments.com/#Activity), (?a http://www.assessments.com/#activityHasReactives ?l), listLength(?l, ?len), lessThan(?len, 2) -> (?x rb:violation error('error', 'Error de conteo de reactivos en actividad', ?a))]");
		set.addRule(rule);

		rule = new Rule();
		rule.setId(5);
		rule.setDescription("Regla compleja de conteo de reactivos en actividad sin validador activado");
		rule.setRule("[(?a rdf:type http://www.assessments.com/#Activity), (?a http://www.assessments.com/#activityHasReactives ?l), listLength(?l, ?len), lessThan(?len, 5) -> (?x rb:violation error('error', 'Error de conteo de reactivos en actividad', ?a))]");
		set.addRule(rule);
		...
		// rule.setRule("[(?a rdf:type http://www.assessments.com/#Assessment), (?a http://www.assessments.com/#assessmentHasActivities ?l), listLength(?l, ?len) -> print('Salida de la evaluación', ?a, ?l, ?len)]");
		// set.addRule(rule);
